...
def main() -> None:
    ...
    colormap = label_colormap(args.face_classes)
    ...
    try:
        ...
        while True:
            ...
            if frame is None:
                ...
                # Parse faces
                start_time = time.time()
                age, masks = age_estimator.predict_img(frame, faces, rgb=False)
                elapsed_time = time.time() - start_time
                ...
                for i, (face, mask) in enumerate(zip(faces, masks)):
                    ...
                    index = mask > 0
                    res = colormap[mask]
                    dst[index] = (1 - alpha) * frame[index].astype(float) + alpha * res[
                        index
                    ].astype(float)
